...
if [[ ! -s ${HYP}.words ]]; then
    ...
    sed "s/\[pause\]//g" ${HYP} > ${HYP}.nopause
    python ${ROOT}/phonemes-eow-to-phoneticwords.py ${HYP}.nopause durations
...
echo "Prism:"
`dirname ${CONDA_PREFIX}`/prism/bin/python ${ROOT}/third_party/prism/prism.py --cand ${HYP}.words --ref ${REF_TEXT} --lang en --model-dir ${ROOT}/third_party/prism/m39v1
echo "COMET:"
...
python count_durations.py ${REF} ${HYP}
